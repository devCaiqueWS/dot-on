...
<div class="panel">
    <h2><?= icon('certificado', 18) ?>Certificado Digital ICP-Brasil (A1)</h2>
    ...
    <?php if ($tem_cert): ?>
        ...
        <form method="post" onsubmit="return confirm('Remover certificado? As próximas assinaturas voltarão a ser SHA-256 sem ICP-Brasil.')">
            <button name="remover" value="1" class="btn btn-danger"><?= icon('trash', 16) ?>Remover certificado</button>
        </form>
        <hr style="margin:20px 0;border:none;border-top:1px solid var(--border)">
        <h3 style="font-size:15px;display:flex;align-items:center;gap:8px"><?= icon('upload', 17) ?>Substituir por outro certificado</h3>
    <?php else: ?>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <p style="font-size:11px;color:#6b7280;margin-bottom:12px;display:flex;align-items:center;gap:6px">
            <?= icon('certificado', 14) ?><span>A senha é cifrada com AES-256-GCM antes de ser armazenada. Apenas o servidor consegue decifrar.</span>
        </p>
        <button class="btn btn-primary"><?= icon('upload', 17) ?>Enviar certificado</button>
    </form>
</div>

<div class="panel">
    <h2><?= icon('info', 18) ?>Sobre certificados ICP-Brasil</h2>
